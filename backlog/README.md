...
| [BL-REL-004](BL-REL-004-the-macos-update-installs-and-then-does-not-restart.md) | The macOS update installs, says "restarting shortly", and never restarts (nothing calls relaunch); and an app on a non-boot volume cannot update at all. Found by the first staging-leg run | Open, needs a decision before the v1.1.0 build is final | High |
