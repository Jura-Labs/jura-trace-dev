...
| R2 | False positive: authentic content incorrectly flagged as manipulated, leading to adverse action against a data subject | Very Low | Medium | LOW | Three-way verdict system avoids binary determinations. Signal Agreement dashboard highlights detector disagreement. Methodology statement in every report. Current test-set false positive rate: 0% (AUC 1.000 on validation set). | Establish internal policy that Jura Trace results are advisory only and must be reviewed by a qualified human before any action is taken. |
| R3 | False negative: manipulated content assessed as authentic, leading to reliance on fabricated media | Low-Medium | High | MEDIUM | Deep and Archival analysis modes run additional detectors and more frames. Trained GBM classifier supplements heuristic scoring. 17+ independent forensic detectors provide defence in depth. | Use Deep or Archival mode for high-stakes analysis. Cross-reference with additional sources. Do not rely on any single tool for critical decisions. |
