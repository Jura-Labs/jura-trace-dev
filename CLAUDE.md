...
- **LimitationBanner**: `ui/src/lib/components/LimitationBanner.svelte` — amber warning banners in Expert View for five input quality conditions (heavy JPEG compression, low resolution, screenshot detected, non-JPEG format, missing GPS/timestamp)
...
**Sprint 28 (7 April 2026)**: Input quality assessment and per-analysis limitation banners. `InputQualityAssessment` struct added to `lib.rs` with eight fields: `jpegQualityEstimate`, `resolutionCategory`, `width`, `height`, `isScreenshotLikely`, `isJpeg`, `hasGps`, `hasTimestamp`, and `degradedDetectors`. Three Rust helper functions: `assess_input_quality()`, `estimate_jpeg_quality()`, `detect_screenshot()`. Wired into verify pipeline after EXIF analysis, before sidecar calls; exposed on `VerificationResult` as `input_quality: Option<InputQualityAssessment>`. New `LimitationBanner.svelte` component renders amber warnings in Expert View for five conditions: heavy JPEG compression (Q<40), low resolution (<0.5 MP), screenshot detected, non-JPEG format, missing GPS/timestamp. `getDetectorApplicability()` helper on verify page returns four status levels — Analysed (malachite), Limited (amber), N/A (grey), Unavailable (cinnabar) — with badges on ELA, Noise, Copy-Move, and Deepfake headings; uses `degradedDetectors` from backend plus `sidecarAvailable` status. WCAG 2.2 AA compliant (role="status", aria-label, dark mode).
...
**7 April 2026 — GBM v4 + UnivFD v8 promoted to production** (commits a7e4b1d, 476f265, 4fa4547, 0857764). GBM Deepfake Classifier retrained three times on 7 April: v2 baseline (9,658 images) → v3 (10,721 — added COCO train set + restored audited Wikimedia) → v4 final (10,709 — Wikimedia re-audit removed 12 outliers: cartoons, microscope slide, album artwork, studio composites, underwater shots). v4 metrics: AUC-ROC 0.9868, authentic FP rate 4.54%, AI detection rate 92.52%, calibrated threshold 0.49, file size ~1.2 MB, SHA-256 `2931f197cba6f376e85b1cbcfd584e6802f36e4fbf68ff00c83d61d4d655db18`. UnivFD probe retrained in parallel: v7 (10,724 images, AUC 0.9909, FP 4.91%) → v8 final (10,712 images, AUC 0.9911, FP 5.01%, recall 96.01%), SHA-256 `d16fb22baf3981d62888e2458733c1a4c0743a5895470d82e9de76766f776908`. MakerNote authenticity bonus added to EXIF anomaly scoring (commit 476f265): images with `MakerNote` EXIF data receive a trust bonus, improving real-camera disambiguation beyond what cross-validation scores show. Camera FP improvements v2→v4: consumer cameras (Pixel/iPhone) 15.6% → 8.81% (-6.79pp, cleared 2× bias gate); high-end cameras (DJI/DSC) 13.5% → 10.32% (still flagged). Bias check top FP source: `wikimedia_photos` 24.80% (n=254, bulk wildlife/insect macro — addressed in Sprint 29 Track 2 with iNaturalist photos). All AI generator families pass: 100% recall on Grok Aurora, ELSA SD, Midjourney v6, SDXL, Gemini Imagen, ArtBench, HF AI; 91.4% DALL-E 3; 75.8% Civitai SFW; 67.6% DiffusionDB (weakest — older SD outputs).

**7 April 2026 — afternoon/evening session**: Sprint 29 delivery and model refresh. GBM classifier retrained three times culminating in v4 (commit `0857764`): 10,709 images (5,724 authentic + 4,985 AI), AUC-ROC 0.9868, overall FP 4.54%, AI recall 92.52%, calibrated threshold 0.49. Wikimedia re-audit removed 12 outliers (cartoon, microscope slide, album artwork, studio composites, underwater shots). Camera FP: consumer (Pixel/iPhone) 15.6% → 8.81% (cleared 2× bias gate); high-end (DJI/DSC) 13.5% → 10.32% (still flagged for future work). UnivFD probe v8 (commit `29317db`): 10,712 images, AUC-ROC 0.9911, FP 5.01%, recall 96.01%. Sprint 29 Track 1 shipped (commit `476f265`): `KNOWN_CAMERA_VENDORS` table covering 36 vendors including 11 Global Majority brands; `camera_authenticity_confidence()` 0.0–1.0; threaded through verify pipeline via `?camera_authenticity_bonus=` sidecar query parameter; max 0.25 score reduction, bypassed when heuristic ≥ 0.75; 6 new exif_anomaly unit tests. KNOWN ISSUE: bonus has no visible UI indicator — `cameraAuthenticityBonus` absent from TypeScript interface; deferred to next sprint. Track 2 partial: wildlife/macro 204 images + war/conflict 203 images downloaded from Wikimedia Commons via new crawler `scripts/agents/crawl_wildlife_and_conflict.py` (commits `8429728`, `3c817c7`); ~1,000 of target 1,200–1,400 Global Majority handset photos still outstanding. Track 3 shipped (commit `52a5690`): 4 new features in `deepfake.py` (`noise_lf_hf_ratio`, `demosaic_inter_channel_coherence`, `noise_anisotropy_mean`, `noise_anisotropy_std`); feature vector 80→84; backwards-compat: classifier loader trims to `clf.n_features_in_`; 24 unit tests in `sidecar/tests/test_new_features.py`; KNOWN LIMITATION on `demosaic_inter_channel_coherence` (DC-dominated sampling, needs future iteration). Track 4 shipped (commit `49a7e8a`): `scripts/build_validation_test_set.py` with `build`/`validate` subcommands, 8 strata; first run 339 images, FP 1.68%, AI recall 99.00%; CRITICAL CAVEAT: currently samples from training corpus — not a true held-out set. Corpus now 6,571 authentic + 5,005 AI = 11,576 total. Composite border detector `scripts/find_composite_borders.py` added; run on full corpus, 21 candidates flagged (0.45% rate) — HTML preview pending user review. Context7 MCP and GitHub MCP installed via `claude mcp add`. Disk cleanup: root disk hit 99% capacity; 33 GB recovered (Hugging Face cache 17 GB, Tauri debug/release targets ~14 GB, pip cache + pycache ~2 GB, sidecar build 229 MB). Documentation: agent persona files refreshed to v4 metrics; model cards page updated to GBM v4.0 + UnivFD v8.0; TRIED roadmap Sprint 29 updated.

**Test counts**: 289 Rust tests, 47 sidecar deepfake tests, 375+ Python tests total, 164 Playwright e2e tests, 231 SvelteKit files with 0 svelte-check errors, clippy + fmt clean.
...
### UI audit items deferred from 7 April 2026

Chrome DevTools + two agents (ux-frontend-designer, content-authenticity-expert) reviewed all key pages on 7 April 2026. Six actionable items deferred to the next sprint:

UI-1. **Fix "Content Credentials" text on dashboard** (`ui/src/routes/+page.svelte` line 186) — missed by the earlier rebranding pass. One-line fix.

UI-2. **R/B/G channel toggle — remove or repair** — the content-authenticity-expert recommends removing the feature entirely and documenting it as an external-tool reference in `/help/methodology`. Rationale: non-experts cannot use it; experts already have the 16 automated detectors. Current bug root cause: `toggleChannel()` samples from `document.querySelector('img[data-preview="true"]')` which is the already-greyscale image after the first toggle. Decision pending: remove or repair.

UI-3. **Remove InspectionChecklist from verify flow, move to help docs** — ux-frontend-designer recommendation. Component is well-built but asks users to do the machine's job manually; content overlaps with automated detector output. Move to Help → Verify Guide. Removes import, `$state`, and approximately 70 lines of template.

UI-4. **Surface MakerNote authenticity bonus in verify UI** — add `cameraAuthenticityBonus: number` to `ExifAnalysis` interface in `types.ts`; render a malachite info badge on the verify page EXIF section when value > 0.5 (e.g. "Camera MakerNote verified — {make} {model}"). Also check whether Severity::Info findings are being hidden by the current UI filter.

UI-5. **Setup wizard null-guard bug** (`ui/src/routes/+layout.svelte` line 47) — `if (setupVersion !== currentVersion)` fires when `setupVersion` is null (older install without stored version), aggressively re-triggering the wizard. Fix: add `&& setupVersion !== null` guard.

UI-6. **Terminology cleanup — three remaining "content credentials" instances**:
   - `ui/src/routes/+page.svelte` dashboard stat label → "C2PA-signed files"
   - `ui/src/routes/monitor/+page.svelte` line 333 narrative text
   - `ui/src/routes/help/glossary/+page.svelte` line 202 primary index term (currently "Content Credentials"; should be "C2PA Provenance" with "Content Credentials" as see-also)
